...
module.exports = function(god, loggerName = 'things') {
    ...
    init: function() {
        ...
        Object.keys(thingDefinitions).forEach(id => thingDefinitions[id].id = id)
    ...
    createThing: function(def) {
        if (def.api == 'tasmota') {
            ...
            this.logger.warn('Registering Tasmota Thing %s (%s)', def.id, mqttTopic)
    ...
    onMqttTasmotaSwitch: async function(trigger, topic, message, packet) {
        ...
        if (topic == 'stat/' + def.device + '/RESULT') {
        ...
        } else if (topic == 'stat/' + def.device + '/STATUS11') {
        ...
        } else if (topic == 'tele/' + def.device + '/STATE') {
            if (newValue.hasOwnProperty(def.power)) {
                newValue = newValue[def.power]
                let oldValue = thing.value
                thing.value = newValue
        ...
        } else if (topic == 'stat/' + def.device + '/' + def.power) {
            let oldValue = thing.value
            thing.value = newValue
            thing.lastUpdated = new Date()
    ...
    addTasmotaSwitchTrigger: function(def) {
        ...
        god.mqtt.addTrigger('stat/' + def.device + '/' + def.power, def.id, this.onMqttTasmotaSwitch)
